...
RMSE_train_DOSE <- predict(m1_10, train_data) %>%
...
RMSE_train_All <- predict(m2_10, train_data) %>%
  bind_cols(train_data) %>%
  metrics(truth=Y, estimate=.pred) %>%
  print()
```
...
RMSE_train_Null <- predict(m3_10, train_data) %>%
  bind_cols(train_data) %>%
  metrics(truth=Y, estimate=.pred) %>%
  print()
```
...
Model 2 performs better than Model 1, as it has a lower RMSE (643.02 vs. 690.78). This suggests that Model 2 has improved predictive accuracy and makes smaller errors in predicting Y. fFr  R², Model 2 has a higher R² (0.5876 vs. 0.5624), meaning it explains more variance in Y.
A higher R² suggests Model 2 fits the data better than Model 1. The result is consistent with original model. 
